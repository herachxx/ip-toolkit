...
> ⚠️ **LEGAL NOTICE:** This tool is for educational purposes and authorized security research only. Only use it against systems and networks you own or have explicit written permission to test. Unauthorized scanning is illegal in most jurisdictions. The author takes no responsibility for misuse.

---

## `> OVERVIEW`

**IP Toolkit** is a modular command-line OSINT tool built entirely in Python with zero third-party dependencies. It combines six investigation modules into one unified interface — giving you geolocation data, open port detection, blocklist reputation checks, WHOIS registration data, network path tracing, and hostname discovery from a single script.
